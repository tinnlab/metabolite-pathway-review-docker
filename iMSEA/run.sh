...
if test -f "$LIC_FILE"; then
    ...
    echo "Please place your license.lic file in the following path: /code/license/license.lic"
    exit 1
fi

# Set Java path (ensure this path is correct)
...
# Create the Matlab script to run the Matlab function
cat <<EOF > /code/run_imsea.m
clear
clc
tic

% Add the directory containing function files to the MATLAB path
addpath('/code')
...
for i = 1:num_seeds
    afterwalk(i, :) = RWR_new(Net, startNodes(i), 1, 0.3);
...
% Calculate the perturbed PA of all groups
...
save('/code/output/PA.mat','CI', 'P_A', 'P_B', 'P_AB', 'pathScore_A','pathScore_B', 'pathScore_AB')
EOF

# Ensure the script can find the necessary files
cd /code

# Run Matlab
matlab -nodisplay -nodesktop -batch "run('main.m'); exit"
